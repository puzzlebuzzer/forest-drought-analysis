...
# Manifest lives inside each index directory: indices/NDVI/cache_manifest.json etc.
# =========================================
...
# ── Generate 6-month search windows ──────────────────────────
windows = []
...
filtered = [i for i in all_raw_items
...
for i, (item, date_str, tile_id) in enumerate(deduped):
    ...
    if (i + 1) % 50 == 0:
        print(f"  [{i+1}/{len(deduped)}] "
              + " | ".join(f"{idx}: {rebuilt[idx]} rebuilt"
                           for idx in INDICES))
...
for idx in INDICES:
    ...
    print(f"  ✓ {idx}/cache_manifest.json  "
          f"({rebuilt[idx]} entries, {missing_file[idx]} files not found)")

# ── Summary ───────────────────────────────────────────────────
print(f"\n{'='*60}")
print("MANIFEST REBUILD SUMMARY")
print(f"{'='*60}")
for idx in INDICES:
    print(f"\n{idx}:")
