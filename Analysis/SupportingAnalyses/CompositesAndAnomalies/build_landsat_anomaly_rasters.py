#!/usr/bin/env python3
"""
build_landsat_anomaly_rasters.py
---------------------------------
Annual anomaly rasters for Landsat C2 NDVI, NDMI, and EVI.  (Axis G3)

Reads the annual maximum composites produced by build_landsat_annual_composites.py
...
and ensure config/wet_dry_years.csv is current (covers 2017-2025; earlier years
treated as all-years baseline when NEUTRAL_YEARS_ONLY=False).

Dependency: run build_landsat_annual_composites.py first.

Output structure:
  Results/rasters/landsat_anomaly/
    ndvi_north_landsat/
      baseline.tif          multi-year mean baseline
      2017.tif  2018.tif    anomaly = year_max - baseline
      ...
    ndvi_south_landsat/  ndmi_north_landsat/  ndmi_south_landsat/  (same structure)

Run from project root:
  python Analysis/SupportingAnalyses/CompositesAndAnomalies/build_landsat_anomaly_rasters.py
"""
...
ANOMALY_DIR    = project_path("landsat_anomaly_dir")
...
print("Landsat C2 Anomaly Rasters")
print("=" * 80)
...
for aoi in AOIS:
    ...
    for index_name in INDICES:
        src_dir = ANNUAL_MAX_DIR / f"{index_name.lower()}_{aoi}_landsat"
        out_dir = ANOMALY_DIR    / f"{index_name.lower()}_{aoi}_landsat"
        out_dir.mkdir(parents=True, exist_ok=True)

        tif_paths = sorted(src_dir.glob("*.tif"))
        if not tif_paths:
            print(f"  SKIP  {index_name}/{aoi} — no composites found in {src_dir}")
            print(f"        Run build_landsat_annual_composites.py first.")
            continue
...
print("\nLandsat C2 anomaly rasters complete.")
