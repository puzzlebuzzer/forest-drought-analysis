...
3. Run:
   ```
   python Analysis/SupportingAnalyses/ArcGISExports/build_mosaic_lpkx.py
   ```

### Option B — from PowerShell / cmd using the full Python path
```powershell
& "C:\Program Files\ArcGIS\Pro\bin\Python\envs\arcgispro-py3\python.exe" `
    Analysis/SupportingAnalyses/ArcGISExports/build_mosaic_lpkx.py
```

### Option C — from within ArcGIS Pro
1. Open the **Catalog pane** → right-click the script → **Run**
   (or use the Python window: `exec(open('Analysis/SupportingAnalyses/ArcGISExports/build_mosaic_lpkx.py').read())`)
